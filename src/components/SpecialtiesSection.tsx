          ...
          {specialties.map((spec) => (
            <motion.div
              ...
              className={`relative rounded-2xl bg-card shadow-card transition-shadow hover:shadow-elevated ${
                spec.wide ? "lg:col-span-7 lg:col-start-4 lg:w-[55.83%] lg:justify-self-center p-6" : "lg:col-span-4 p-6"
              }`}
            >
              ...
              {spec.wide ? (
                <div className="flex flex-col">
                  {spec.badge && (
                    <span className="absolute top-4 right-8 inline-flex items-center gap-1 rounded-full bg-accent/10 px-3 py-1 text-xs font-medium text-accent">
                      <Badge size={12} strokeWidth={1.5} />
                      {spec.badge}
                    </span>
                  )}
